...
export default function SetupGuide({ embedded = false }) {
  const body = (
    <div className={embedded ? 'space-y-2' : 'border-t border-blue-400/20 p-3 space-y-2'}>
      <p className="text-[12px] text-slate-300 leading-relaxed">
        Repeat these steps for every new client. Studio panel + this guide are hidden on
        client-facing deploys (set{' '}
        <code className="font-mono text-[11px] bg-black/40 px-1 rounded">
          VITE_SHOPSITE_STUDIO_MODE=off
        </code>
        ).
      </p>
      ...
      <Step n="1" title="Duplicate the template for the client">
        <p>
          Copy the repo (or use a fresh branch / Cloudflare Pages project per client). Each
          client gets their own deploy with their own env.
        </p>
        <CodeBlock>{`# Example
...
VITE_SITE_TITLE=Acme — Your store
VITE_SITE_META_DESCRIPTION=Short SEO description of what Acme sells.
...
VITE_SHOPSITE_STUDIO_MODE=on`}</CodeBlock>
      ...
      <Step n="3" title="Edit copy in Studio → Copy (or `src/content/*.js`)">
        <p>
          Hero, home sections, pricing, FAQ, store, sales, blog — everything reads from{' '}
          <code className="font-mono text-[11px]">src/content/*.js</code>. Use the{' '}
          <strong>Copy</strong> tab to edit live (click sample fields to select &amp; replace).
          For permanent client changes, merge into the source files.
        </p>
      </Step>

      <Step n="4" title="In your Stripe Dashboard, create products + prices">
        <p>
          Create one Product per plan, each with a recurring Price (or one-time). Copy each{' '}
          <code className="font-mono text-[11px]">price_…</code> ID.
        </p>
        <p>
          Paste into <code className="font-mono text-[11px]">src/content/pricing.js</code> as{' '}
          <code className="font-mono text-[11px]">cta.stripePriceId</code>. Use{' '}
          <code className="font-mono text-[11px]">stripeCheckoutMode: &apos;subscription&apos;</code>{' '}
          or <code className="font-mono text-[11px]">&apos;payment&apos;</code>.
        </p>
      </Step>
      ...
      <Step n="5" title="Onboard the client to Stripe Connect">
        <p>
          Open{' '}
          <a className="text-blue-300 underline" href="/seller-onboarding">
            /seller-onboarding
          </a>{' '}
          on this site. Click <em>Create Stripe account</em>, then <em>Continue to Stripe</em>. The
          client adds bank + identity. After return, copy the{' '}
          <code className="font-mono text-[11px]">acct_…</code> ID.
        </p>
      </Step>

      <Step n="6" title="Set Connect destination + your fee">
        <p>
          Add to <code className="font-mono text-[11px]">.env.local</code> for this client:
        </p>
...
STRIPE_CONNECT_APPLICATION_FEE_AMOUNT=100    # one-time payments cut, in cents`}</CodeBlock>
        <p>
          Funds settle to <strong>their</strong> bank; your <em>application fee</em> lands in your
          platform balance.
        </p>
      </Step>
...
npx wrangler deploy`}</CodeBlock>
        <p>
          ...
          <code className="font-mono text-[11px]">
            https://shopsite-stripe-api-acme.&lt;account&gt;.workers.dev
          </code>
          .
        </p>
      </Step>
      ...
      <Step n="8" title="Point the site at the Worker">
        <p>
          Add this to <code className="font-mono text-[11px]">.env.local</code> (and the host&apos;s
          env vars when deploying):
        </p>
        <CodeBlock>{`VITE_STRIPE_API_BASE=https://shopsite-stripe-api-acme.<account>.workers.dev`}</CodeBlock>
        <p>
          Both <code className="font-mono text-[11px]">/checkout</code> and{' '}
          <code className="font-mono text-[11px]">/seller-onboarding</code> will hit that URL.
        </p>
      </Step>
...
VITE_SHOPSITE_STUDIO_MODE=off`}</CodeBlock>
        <p>
          This hides the gear icon, this checklist, and removes{' '}
          <code className="font-mono text-[11px]">/seller-onboarding</code> from the router.
        </p>
        <p>
          Quick toggle (your machine only): visit{' '}
          <code className="font-mono text-[11px]">?studio=on</code> or{' '}
          <code className="font-mono text-[11px]">?studio=off</code>.
        </p>
      </Step>

      <Step n="10" title="Smoke test in test mode first">
        <p>
          With <code className="font-mono text-[11px]">pk_test_/sk_test_</code> keys: open{' '}
          <code className="font-mono text-[11px]">/checkout</code>, run a card{' '}
          <code className="font-mono text-[11px]">4242 4242 4242 4242</code>, any future date, any
          CVC. Confirm:
        </p>
        <ul className="list-disc list-inside ml-2 space-y-1">
          <li>Embedded checkout loads in a modal.</li>
          <li>
            Stripe Dashboard shows the Charge with <em>Application fee</em> on your platform.
          </li>
...
# Vercel/Netlify: same idea`}</CodeBlock>
        <p>
          Set the same <code className="font-mono text-[11px]">VITE_*</code> vars in the host&apos;s
          project settings (do not rely on <code className="font-mono text-[11px]">.env.local</code>{' '}
          in production).
        </p>
      </Step>
      ...
      <Step n="12" title="Hand-off">
        <ul className="list-disc list-inside ml-2 space-y-1">
          ...
          <li>
            Confirm{' '}
            <code className="font-mono text-[11px]">VITE_SHOPSITE_STUDIO_MODE=off</code> on the live
            deploy.
          </li>
          <li>
            Bookmark <code className="font-mono text-[11px]">/seller-onboarding?studio=on</code> for
            yourself if you ever need to re-run setup.
          </li>
        </ul>
      </Step>
    </div>
  ...
  if (embedded) {
    return (
      <div>
        <h3 className="mb-1 text-sm font-bold text-white">Client site setup checklist</h3>
        <p className="mb-3 text-[11px] text-slate-500">Studio</p>
        {body}
      </div>
    );
  }
